...
const AdminProfile = () => {
  ...
  const [newGym, setNewGym] = useState({ ID: uuidv4(), Name: '', Phone: '', City: '', Addres: '', IsChain: true });
  ...
  const handleAddGym = () => {
    axios.post('http://localhost:8080/api/v1/gym/new', newGym)
      .then(_ => {
        setGyms([...gyms, newGym]);
        setNewGym({ ID: uuidv4(), Name: '', Phone: '', City: '', Addres: '', IsChain: true });
      })
      .catch(error => {
  ...
  const handleAddTrainer = () => {
    axios.post('http://localhost:8080/api/v1/trainer/new', newTrainer)
      .then(response => {
        ...
        setNewTrainer({ ID: uuidv4(), Fullname: '', Email: '', Phone: '', Qualification: '', UnitPrice: '', GymsID: [] });
      })
      .catch(error => {
  ...
  const handleAddMembershipType = (gymId) => {
    const membershipType = { ...newMembershipType, GymID: gymId, DaysDuration: Number(newMembershipType.DaysDuration) };
  ...
  const renderGyms = () => (
    <div>
      ...
      <div className="new-gym-form">
        ...
        <label>
          Адрес:
          <input type="text" value={newGym.Addres} onChange={(e) => setNewGym({ ...newGym, Addres: e.target.value })} />
        </label>
        ...
        <div className="gym-management">
          ...
          <div>
            ...
            <div>
              ...
              <label>
                Длительность (дней):
                <input type="number" value={newMembershipType.DaysDuration} onChange={(e) => setNewMembershipType({ ...newMembershipType, DaysDuration: Number(e.target.value) })} />
              </label>
